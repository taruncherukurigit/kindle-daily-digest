"""
Lightweight receiver: accepts the finished digest text from n8n via
HTTP POST, renders it to EPUB with pandoc, archives a dated copy,
sends it via send_digest.py, and provides a full-article-text
extraction endpoint (used by the n8n digest-assembly step) built on
trafilatura.
...
def weekly_archive():
    ...
    returns them concatenated as plain text. Exists because n8n runs
    on a different container and has no filesystem access to
    digest-archive/ — this endpoint is the only thing that CAN read
    it, so it reads locally and hands the result back over HTTP."""
    import time
...
def extract():
    ...
    (not an error) on any failure, so the n8n caller can fall back
